                    ...
                    axios.get("http://localhost:3000/tasks?_sort=deadline")
                        .then(async response => {
                            ...
                            for(let i = 0; i < tasks.length; i++){
                                if(parseInt(tasks[i].id) > maxTaskID) maxTaskID = parseInt(tasks[i].id)
                            }
                            maxTaskID++
                            ...
                            for(let i = 0; i < users.length; i++){
                                if(parseInt(users[i].id) > maxUserID) maxUserID = parseInt(users[i].id)
                            }
                            maxUserID++

                            res.writeHead(200, {'Content-Type': 'text/html;charset=utf-8'})
                            res.write(templates.mainPage(users, tasks, maxUserID.toString(), maxTaskID.toString()))
                            res.end()
                        })
                        .catch(function(erro){
                            res.writeHead(200, {'Content-Type': 'text/html;charset=utf-8'})
                ...
                // GET /alunos/registo --------------------------------------------------------------------
                    ...
                    axios.get("http://localhost:3000/tasks/" + idTask)
                        .then( async response => {
                            ...
                            var users = await getUsers()
                            res.writeHead(200, {'Content-Type': 'text/html;charset=utf-8'})
                            res.end(templates.editPage(a, users)) 
                        })
                        .catch(function(erro){
                            res.writeHead(200, {'Content-Type': 'text/html;charset=utf-8'})
